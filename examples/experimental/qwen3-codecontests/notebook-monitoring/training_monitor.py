...
def _chart_arrays(s):  # (xs, step_t, roll_t, reward, trunc) aligned by step
    xs = sorted(s)
    return (xs,
...
def _new_figure(go, make_subplots, widget=False):
    """3-subplot figure with 4 empty traces. widget=True -> mutable go.FigureWidget."""
...
def _fill_traces(fig, s):
    xs, step_t, roll_t, reward, trunc = _chart_arrays(s)
    for tr, y in zip(fig.data, (step_t, roll_t, reward, trunc)):
        tr.x, tr.y = xs, y
...
def step_table(s):  # text table of per-step metrics (printed in the text region)
    xs, step_t, roll_t, reward, trunc = _chart_arrays(s)
    lines = [f"{'step':>4} {'step_t':>7} {'roll_t':>7} {'reward':>7} {'trunc':>6}"]
